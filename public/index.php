...
include '../src/controllers/ctrlFilterEvents.php';
...
include "../src/controllers/ctrlmap.php";
...
include "../src/controllers/ctrlaboutus.php";
...
include "../src/controllers/ctrlDashboardUsers.php";
include "../src/controllers/ctrlDashboardEventsEdits.php";
// Archivos Middleware
...
if (!isset($r)) {
  ...
  $response = ctrlaboutus($request, $response, $container);
}elseif($r == "dashboard"){
  $response = isAdmin($request, $response, $container, "ctrlDashboard");
}elseif ($r == "dashboardusers") {
...
//   $response = ctrlEventos($request, $response, $container);
  ...
  $response = isLogged($request, $response, $container, "ctrlEventList");
} elseif ($r == "json") {
  $response = ctrlJson($request, $response, $container);
} elseif ($r == "eliminarevento") {
  $response = ctrlDeleteEvent($request, $response, $container);
} elseif ($r == "editarevento") {
  $response = ctrlEditEvent($request, $response, $container);
} elseif ($r == "dashboardevents") {
  $response = ctrlDashboardEventsEdits($request, $response, $container);
} elseif ($r == "filterevents") {
  $response = ctrlFilterEvents($request, $response, $container);
} elseif ($r == "buscareventos") {
  $response = ctrlSearchEvents($request, $response, $container);
} elseif ($r == "map") {
  $response = ctrlMap($request, $response, $container);
} else {
