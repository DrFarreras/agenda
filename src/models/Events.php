...
class Events
{
    ...
    public function updateEvent($event_id, $event_title, $event_description, $event_location, $event_type, $event_comment, $date_start, $date_end, $event_time){
        $event_comment_value = ($event_comment === 'on') ? 1 : 0;
    
        $query = "UPDATE events SET event_title = :event_title, event_description = :event_description, event_location = :event_location, event_type = :event_type, event_comment = :event_comment, date_start = :date_start, date_end = :date_end, event_time = :event_time WHERE event_id = :event_id";
        $stm = $this->sql->prepare($query);
        $stm->execute([
            ":event_id" => $event_id,
    ...
    public function getAllEvents(){
        ...
        $query = "SELECT event_id, event_title, event_description, event_location, date_start, event_time from events order by date_start desc limit 3;";
        $results = [];
